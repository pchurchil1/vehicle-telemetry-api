.PHONY: up down build migrate seed test test-postgres logs shell
...
test-postgres:
	...
	docker compose exec -e ENVIRONMENT=test -e RUN_POSTGRES_TESTS=1 api pytest -q -m postgres --no-cov
